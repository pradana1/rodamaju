...
class Checkout extends MY_Controller
{
    ...
    public function index($input = null)
    {
        ...
        $data['cart']   = $this->checkout->select(
            [
                'cart.id', 'cart.datebook', 'service.title',
            ]
        )
            ->join('service')
        ...
        if (!$data['cart']) {
            $this->session->set_flashdata('warning', 'Tidak ada service dalam keranjang belanja.');
            redirect(base_url('/'));
        }
    ...
    public function create()
    {
        ...
        $datebook = $this->db->select('datebook')->where('id_user', $this->id)->get('cart')->row()->datebook;
        ...
        if ($order = $this->checkout->create($data)) {
            ...
            foreach ($cart as $row) {
                $row['id_bookings']   = $order;
                unset($row['id'], $row['id_user']);
                $this->db->insert('bookings_detail', $row);
            }
